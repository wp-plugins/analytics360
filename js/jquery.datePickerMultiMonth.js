...
(function($){
	...
	$.fn.extend({
						...
						for (var i=0; i<s.numMonths; i++)
						{
							(function(i) {
								...
								var $date =  $('<div></div>')
												...
												.bind('dateRangeSelected', function(event, start, end) {
													if (first)	{
														$dpmm.trigger('dateRangeSelected', [start, end]);
													} else {
														pickers[i-1].dpSetSelectedRange(start.asString(), end.asString(), false);
													}
													if (!last)	{
														pickers[i+1].dpSetSelectedRange(start.asString(), end.asString(), false);
													}
		...
		dpmmSetSelectedRange: function(start, end, moveToMonth, dispatchEvents) {
			var basePicker = $(this).data('dpBasePicker');
			return basePicker.dpSetSelectedRange(start, end, moveToMonth, dispatchEvents);
		},
		dpmmSetSelected: function(d, v, m, e) {
			var basePicker = $(this).data('dpBasePicker');
			return basePicker.dpSetSelected(d, v, m, e);
		},
		dpmmClearSelected: function() {
			var basePicker = $(this).data('dpBasePicker');
			var c = _getController(basePicker[0]);
			if (c) {
				return c.clearSelected();
			}
			return null;
			
			//return basePicker.dpClearSelected(d, v, m, e);
		}
